...
The `sync-cv.yml` workflow checks for changes daily using a read-only SSH deploy key scoped to the resume repository and stored as the `RESUME_REPO_SSH_KEY` Actions secret. A workflow in the resume repository publishes CV changes immediately after every push to `main`; the daily run is a fallback.
